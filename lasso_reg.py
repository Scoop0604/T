...
def datacleaning(dt):
    ...
    print(dt.iloc[:, 0:len(dt.columns[:])-1])
...
def lasso(a,name,x_train, x_test, y_train,y_test):

    ridge = linear_model.Lasso(alpha=a, fit_intercept=True) 
    ridge.fit(x_train, y_train)
    print(ridge.coef_)
    ridge_coef = pd.DataFrame(ridge.coef_)
    ridge_coef.to_excel(r'D:\case\new-case\lassocoef' + name + '.xlsx')
    ...
    plt.legend(loc="best")
    plt.savefig(r'D:\case\new-case\lasso_predict'+name+'.jpg')
    plt.show()
    return  y_pred

def lasso_alphafig(dt,name):
    x, y = dt.iloc[:, 0:len(dt.columns[:])-1], dt.iloc[:, len(dt.columns[:])-1]
    print(x)
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3, random_state=0)
    n_alphas = 200
...
def lassocv(dt):
    ...
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3, random_state=0)
    n_alphas = 200
    alphalist = list(np.logspace(-4, 4, n_alphas))
    ...
    print('The optimal alpha value',ridge.alpha_)
    a = ridge.alpha_
    return a
...
def interaction_research(dt):
    ...
    # dt['subway_distance'] = dt.subway * dt.distance
    ...
    name2 = 'interaction'
    lasso_alphafig(dt,name2)
    alpha2 = lassocv(dt)
    y_pred_inter = lasso(alpha2,name2,x_train1, x_test1, y_train1, y_test1)
    mse(y_test1,y_pred_inter)


def main(dat,x_train, x_test, y_train, y_test):
    name1 = 'none'
    lasso_alphafig(dat,name1)
    # ridgecv(dt)
    alpha1 = lassocv(dat)
    y_pred3 = lasso(alpha1,name1,x_train, x_test, y_train,y_test)
